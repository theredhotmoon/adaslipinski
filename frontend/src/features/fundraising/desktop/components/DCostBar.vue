...
<template>
  <div :style="{ background: c.surface, border: `1px solid ${c.line}`, borderRadius: '18px', padding: big ? '26px' : '20px' }">
    <div class="flex justify-between items-baseline mb-3">
      <span :style="{ fontWeight: 800, color: c.ink, fontSize: '16px' }">Koszt rehabilitacji / mies.</span>
    ...
    <div class="flex justify-between mt-3" :style="{ fontSize: '14px' }">
      ...
      <span :style="{ color: c.primaryDeep, fontWeight: 800 }">brakuje {{ zl(b.gap) }} miesięcznie</span>
    </div>
  </div>
</template>
